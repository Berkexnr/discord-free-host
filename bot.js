...
client.on("ready", () => {
  client.user.setGame(prefix + "yardım | Herkesin 10 Kasımı Kutlu Olsun") 
...
client.on('message', msg => {
  if (msg.content.toLowerCase() === prefix + 'yardım') {
    ...
    msg.channel.sendEmbed(ozelmesajkontrol) }
      msg.author.sendMessage('Sanırım yardım istedin? **o!!komutlar** yazarak **komutlara bakabilirsin** Başka bir konuda **yardım** istiyorsan **moderatörlerimiz** sana yardımcı olacaktır.').then(message => console.log(`[${moment().format('YYYY-MM-DD HH:mm:ss')}] Gönderilen mesaj: ${message.content}`)).catch(console.error);
  }
});
